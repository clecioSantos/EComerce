...
import type { CreateProductInput, ProductVariantInput } from "./schemas";
...
function hasStock(variants: { inventory: { quantityOnHand: number; quantityReserved: number; allowBackorder: boolean } | null }[]): boolean {
...
export function mapProductDetail(product: ProductWithDetail): ProductDetailDTO {
  ...
  const variants: VariantDTO[] = product.variants.map((variant) => {
    ...
    return {
      ...
      sku: variant.sku,
      name: variant.name,
      price: variant.price == null ? toNumber(product.basePrice) : toNumber(variant.price),
      compareAtPrice:
  ...
  const assignments: ProductAssignmentDTO[] = product.assignments.map(
    (assignment) => ({
    ...
    data: {
      ...
      assignments: {
        create: input.assignments.map((assignment) => ({
          ...
          value: (assignment.value ?? undefined) as
